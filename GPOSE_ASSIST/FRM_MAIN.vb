...
#Region "通信用"
    ...
    Private Enum ENM_EVENT_DETAIL_INDEX
        E_NAME = 1
        E_DETAIL
        E_VALUE
    End Enum
    ...
    Private Sub SUB_PROCESS_MESSAGE_EVENT(ByVal STR_EVENT_DETAIL As String)
        ...
        Dim STR_EVENT_DETAIL_ROW() As String
        STR_EVENT_DETAIL_ROW = FUNC_GET_EVENT_DETAIL_ROW(STR_EVENT_DETAIL)

        Select Case STR_EVENT_DETAIL_ROW(ENM_EVENT_DETAIL_INDEX.E_NAME)
            Case "SEND"
                Select Case STR_EVENT_DETAIL_ROW(ENM_EVENT_DETAIL_INDEX.E_DETAIL)
                    Case "KEY"
                        Select Case STR_EVENT_DETAIL_ROW(ENM_EVENT_DETAIL_INDEX.E_VALUE)
                            Case "W"
                    ...
                    Case "CHAT"
                        ...
                        Call SUB_SEND_KEYS_RETURN(PRC_APP_TARGET)
                        Dim STR_CHAT As String
                        STR_CHAT = STR_EVENT_DETAIL_ROW(ENM_EVENT_DETAIL_INDEX.E_VALUE)
                        Call FUNC_SEND_KEYS_STRING_02(PRC_APP_TARGET, STR_CHAT)
                        Call SUB_SEND_KEYS_RETURN(PRC_APP_TARGET)
                    Case Else
                End Select
            Case "CAPTURE"
    ...
    Private Function FUNC_GET_EVENT_DETAIL_ROW(ByVal STR_EVENT_DETAIL As String) As String()
        Dim STR_RET() As String
        ReDim STR_RET(0)

        Dim STR_TEMP_ROW() As String
        STR_TEMP_ROW = STR_EVENT_DETAIL.Split("_")

        If STR_TEMP_ROW Is Nothing Then
            Return STR_RET
        End If

        For i = 0 To (STR_TEMP_ROW.Length - 1)
            Dim INT_INDEX As Integer
            INT_INDEX = STR_RET.Length
            ReDim Preserve STR_RET(INT_INDEX)
            STR_RET(INT_INDEX) = STR_TEMP_ROW(i)
        Next
